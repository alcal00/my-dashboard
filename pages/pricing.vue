...
import { getPricing, empty_pricing } from "~~/src/pricing/data"
...
const pricing = ref(empty_pricing)
            ...
            <UPricingCard v-for="p in pricing.plans" :title="p.title" :description="p.description"
                ...
                <template #bottom>
                    <a ref="redirectBuyButton"
                        :href="pricingLoaded ? lemonSqueezyURL(isYearly ? p.checkout.yearly : p.checkout.monthly, user, url.origin) : ''"
                        class="lemonsqueezy-button">
                        <UButton :label="pricingLoaded ? checkoutButtonLabel(p) : ''" block
