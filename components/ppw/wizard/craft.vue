...
// import is not working, we get a "self is not defined error"
// so we use a head script to local version of json-editor :
// see https://github.com/json-editor/json-editor/tags
// place it under public/lib/jsoneditor.min-x.x.x.js
// symlink to public/lib/jsoneditor.min.js
import { JSONEditor } from "@json-editor/json-editor/dist/nonmin/jsoneditor.js"
import type { DocumentInfo } from '~/models/document';
/*
useHead({
  script: [
    // Rely on a CDN or host the dep by ourself ?
    //"https://cdn.jsdelivr.net/npm/@json-editor/json-editor@latest/dist/jsoneditor.min.js",
    //"https://cdn.jsdelivr.net/npm/@json-editor/json-editor@2.10.1/dist/jsoneditor.min.js",
    //TODO: the public content does not seem to be exported/included correclty
    // public from paxpar-widgets has to be specified in packages.json/files
...
async function craftLoadFromDocInfo(docInfo: DocumentInfo) {
  ...
  //console.debug('document info', { res })
}

// load craft_from from a given craft_id
async function craftLoadFromId(craft_id: string) {
  //console.debug('craftLoadFromIdload ....')
  // load craft info
  const { data } = await useFetchPP(`/api/craftform/${craft_id}`)
  //console.debug('craftLoadFromIdload data=', data)
  craftId.value = craft_id
  craftInfo.value = data.value as CraftInfo
...
async function craftColdLoad() {
  ...
  if (props.documentInfo !== undefined) {
...
async function generateDocument({ factice }) {
  ...
  const { data, error } = await useFetchPP(`/api/craft/${props.craft_id}`, {
    method: "POST",
    body: { data_schema: form_value },
    params: {
      finalize: !factice,
      options: generate_options.join(","),
    },
    toast: {
      success: {
        msg: factice ? "Document généré" : "Document final généré",
      },
      error: {
        msg: "Erreur lors de la génération du document",
      },
    },
  })
  // move fetch result to refs
  generateFetchData.value = data.value
  generateFetchError.value = error.value
  ...
  if (showCheckStep.value === true) {
    ...
    const data = await useFetchPP(`/api/pdf/check/${craftInfo.value.checklist}`, {
      ...
      body: {},
      params: {
        source_session: outputPdfPath.value,
      },
      toast: {
        success: {
          msg: factice ? "Document vérifié" : "Document final vérifié",
        },
        error: {
          msg: "Erreur lors de la vérification du document généré",
        },
      },
    })
...
async function downloadItem({ url, label }) {
  ...
  const { data } = await useFetchPP(url, { responseType: 'blob' })
...
<template>
  <div class="flex-col p-2 m-2">
    ...
    <div class="modal">
      <div class="modal-box w-11/12 max-w-5xl h-5/6">
        ...
        <div class="modal-action">
          <label for="preview-modal" class="btn">Fermer</label>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-row">
      <ppw-gallery-icon :def="craftInfo?.image2 || craftInfo?.image" class="w-48" />
      <div>
        <p class="text-2xl">{{ craftInfo?.name }}
          <UButton to="/shortcut" variant="ghost" color="gray" icon="mdi:qrcode" class="mx-2" target="_blank" />
        </p>
        <p>{{ craftInfo?.desc }}</p>
      </div>
    </div>
        ...
        <slot :name="`10-drop`">
          ...
          <ClientOnly>
            ...
            <ppw-dropfile-main2 ref="refdropfile" :uploadUrl="`/api/craft/file/${props.craft_id}`"
              :sessionPath="`craft/${props.craft_id}`" :dropfile_webcam="craftInfo?.dropfile_webcam"
              @uploadSuccess="onFileChange" @uploadError="onFileUploadError" />
          </ClientOnly>
        </slot>
      </div>
